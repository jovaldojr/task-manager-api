...
RSpec.describe 'Tak API', type: :request do 
	...
	describe 'GET /taks' do
		...
		context 'when filter and sorting params are sent' do
			...
			it 'returns only the tasks matching' do
				returned_task_title = json_body[:data].map { |t| t[:attributes][:title] }

				expect(returned_task_title).to eq([notebook_task_2.title, notebook_task_1.title])
			end
		end
	end
	...
	describe 'POST /tasks/' do
		...
		context 'whe the params are valid' do
			...
			it 'returns the json for create task' do
				expect(json_body[:data][:attributes][:title]).to eq(task_params[:title])
			end

			it 'assigns the create task to the current user' do
	...
	describe 'PUT /tasks/:id' do
		...
		context 'when the params are invalid' do
			...
			it 'returns json erros for title' do
				expect(json_body[:errors]).to have_key(:title)
			end
	...
	describe 'DELETE /tasks/:id' do
		...
		it 'remove de tasks from the database' do
